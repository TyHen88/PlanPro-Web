...
import { ImageWithLoading } from "@/components/ui/demo/Image-with-landing"
...
export default function BananaSportswearStorefront() {
    ...
    const generatePersonalizedImagesWithFile = async (file: File) => {
        ...
        try {
            ...
            alert(`Failed to generate personalized images: ${errorMessage}. Please try again.`)
        } finally {
    ...
    return (
        ...
        >
            <div
                className={`fixed bottom-8 right-8 z-40 transition-all duration-700 ${isPageLoaded ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"
                    }`}
                style={{ transitionDelay: "800ms" }}
            >
                <div
                    className={`border-2 border-dashed transition-all duration-300 ${isDragOver ? "border-black bg-white shadow-lg scale-105" : "border-gray-300 bg-gray-50"
            ...
            <header
                className={`px-8 py-6 border-b border-gray-200 transition-all duration-700 ${isPageLoaded ? "translate-y-0 opacity-100" : "-translate-y-4 opacity-0"
                    }`}
            >
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    ...
                    <nav className="hidden md:flex items-center space-x-12">
                        {["NEW", "MEN", "WOMEN", "KIDS"].map((item, index) => (
                            <a
                                key={item}
                                href="#"
                                className={`text-black hover:text-gray-500 text-xs font-medium tracking-widest uppercase transition-all duration-500 ${isPageLoaded ? "translate-y-0 opacity-100" : "translate-y-2 opacity-0"
                                    }`}
                        ...
                        <Button
                            variant="outline"
                            size="sm"
                            className={`border-black text-black hover:bg-black hover:text-white text-xs font-medium tracking-widest uppercase bg-transparent px-6 transition-all duration-500 ${isPageLoaded ? "translate-y-0 opacity-100" : "translate-y-2 opacity-0"
                                }`}
                    ...
                    <div
                        className={`flex items-center space-x-6 transition-all duration-700 ${isPageLoaded ? "translate-x-0 opacity-100" : "translate-x-4 opacity-0"
                            }`}
            ...
            <section className="px-8 py-16">
                <div className="max-w-4xl mx-auto">
                    <div
                        className={`flex items-center justify-between mb-16 transition-all duration-700 ${isPageLoaded ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"
                            }`}
                            ...
                            >
                                <div className="relative w-full overflow-hidden mb-4">
                                    <ImageWithLoading
                                        ...
                                        className={`w-full h-auto object-contain group-hover:scale-105 transition-all duration-500 ${isTransitioning ? "opacity-90" : "opacity-100"
                                            }`}
            ...
            <footer
                className={`border-t border-gray-200 px-8 py-16 bg-gray-50 transition-all duration-700 ${isPageLoaded ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"
                    }`}
